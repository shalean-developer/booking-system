-- ============================================
-- FIX WALIED COOKSON'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Every Thursday (weekly)
-- Cleaner: Maggert Jiri
-- Total Cost: R451.56 (45156 cents)
-- Cleaner Earnings: R300 (30000 cents)
...
WHERE c.email = 'walied44@gmail.com'
...
SET 
  ...
  total_amount = 45156, -- R451.56 in cents
  cleaner_earnings = 30000, -- R300 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Maggert Jiri' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'walied44@gmail.com' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Maggert Jiri' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      45156 as total_amount, -- R451.56 in cents
      30000 as cleaner_earnings, -- R300 in cents
    ...
    RAISE NOTICE 'Created new schedule for Walied Cookson with ID: %', v_schedule_id;
  END IF;
END $$;
...
SELECT 
  ...
  ws.address_suburb,
  ws.address_city,
  ws.total_amount_cents,
  ws.cleaner_earnings_cents,
  ws.cleaner_id,
  ws.schedule_id,
  'pending' as status,
  jsonb_build_object(
    'service_type', ws.service_type,
    'bedrooms', ws.bedrooms,
    'bathrooms', ws.bathrooms,
    'extras', '[]'::jsonb,
    'extrasQuantities', '{}'::jsonb,
    'subtotal', ROUND(ws.total_amount_cents / 100.0 - 50, 0),
    'serviceFee', 50,
    'frequencyDiscount', 0,
    'total', ROUND(ws.total_amount_cents / 100.0, 2),
