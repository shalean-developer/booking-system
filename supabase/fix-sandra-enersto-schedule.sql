...
WHERE c.email = 'sandraernesto21@yahoo.com'
...
SET 
  ...
  total_amount = 52750, -- R527.50 in cents
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'sandraernesto21@yahoo.com' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      52750 as total_amount, -- R527.50 in cents
    ...
    RAISE NOTICE 'Created new schedule for Sandra Enersto with ID: %', v_schedule_id;
  END IF;
END $$;
...
SELECT 
  ...
  jsonb_build_object(
    ...
    'subtotal', ROUND(ss.total_amount_cents / 100.0 - 50, 0),
    'serviceFee', 50,
    'frequencyDiscount', 0,
    'total', ROUND(ss.total_amount_cents / 100.0, 2),
