-- ============================================
-- FIX ROGEN MOODLEY'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Every Tuesday and Friday (custom-weekly)
-- Cleaner: Lucia Pazvakavambwa
-- Total Cost: R350.00 (35000 cents)
-- Cleaner Earnings: R250 (25000 cents)
-- Start Date: October 1, 2025 (from image)
...
WHERE c.email = 'rogen@iafrica.com'
...
SET 
  ...
  days_of_week = ARRAY[2, 5], -- Tuesday = 2, Friday = 5
  preferred_time = '08:30:00', -- From image: Custom Weekly at 08:30:00
  start_date = '2025-10-01',
  total_amount = 35000, -- R350.00 in cents
  cleaner_earnings = 25000, -- R250 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Lucia Pazvakavambwa' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'rogen@iafrica.com' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Lucia Pazvakavambwa' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      ARRAY[2, 5] as days_of_week, -- Tuesday, Friday
      '08:30:00' as preferred_time,
      v_cleaner_id,
      true as is_active,
      '2025-10-01' as start_date,
      35000 as total_amount, -- R350.00 in cents
    ...
    RAISE NOTICE 'Created new schedule for Rogen Moodley with ID: %', v_schedule_id;
  END IF;
END $$;
...
-- Step 4: Create bookings for all Tuesdays and Fridays in Oct, Nov, Dec 2025
WITH date_series AS (
...
tuesday_friday_dates AS (
  SELECT booking_date
  FROM date_series
  WHERE EXTRACT(DOW FROM booking_date) IN (2, 5) -- Tuesday = 2, Friday = 5
),
rogen_schedule AS (
  SELECT 
    rs.id as schedule_id,
...
SELECT 
  ...
  rs.address_suburb,
  rs.address_city,
  rs.total_amount_cents,
  rs.cleaner_earnings_cents,
  rs.cleaner_id,
  rs.schedule_id,
  'pending' as status,
  jsonb_build_object(
    ...
    'subtotal', ROUND(rs.total_amount_cents / 100.0 - 50, 0),
    'serviceFee', 50,
    'frequencyDiscount', 0,
    'total', ROUND(rs.total_amount_cents / 100.0, 2),
    'snapshot_date', NOW()::text,
    'manual_pricing', true
  ) as price_snapshot,
  NOW() as created_at,
  NOW() as updated_at
FROM tuesday_friday_dates tfd
CROSS JOIN rogen_schedule rs;
