-- ============================================
-- FIX MEGAN BOWREN'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Every Thursday (weekly)
-- Cleaner: Nicole James
-- Total Cost: R400.51 (40051 cents)
...
WHERE c.email = 'meganbowren@gmail.com'
...
SET 
  ...
  total_amount = 40051, -- R400.51 in cents
  cleaner_earnings = 28000, -- R280 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Nicole James' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'meganbowren@gmail.com' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Nicole James' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      40051 as total_amount, -- R400.51 in cents
    ...
    RAISE NOTICE 'Created new schedule for Megan Bowren with ID: %', v_schedule_id;
  END IF;
END $$;
...
-- Step 4: Create bookings for all Thursdays in Oct, Nov, Dec 2025
WITH date_series AS (
