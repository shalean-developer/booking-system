-- ============================================
-- FIX GUGU KALI'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Bi-weekly (every other Saturday)
-- Cleaner: Normatter Mazhinji
-- Total Cost: R464.00 (46400 cents)
-- Cleaner Earnings: R270 (27000 cents)
...
WHERE c.email = 'gugukl@yahoo.co.uk'
...
SET 
  frequency = 'bi-weekly',
  day_of_week = 6, -- Saturday = 6
  preferred_time = '09:00:00',
  start_date = '2025-10-25', -- From image: 25 Oct 2025
  total_amount = 46400, -- R464.00 in cents
  cleaner_earnings = 27000, -- R270 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Normatter Mazhinji' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'gugukl@yahoo.co.uk' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Normatter Mazhinji' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      v_cleaner_id,
      true as is_active,
      '2025-10-25' as start_date,
      46400 as total_amount, -- R464.00 in cents
      27000 as cleaner_earnings, -- R270 in cents
    ...
    RAISE NOTICE 'Created new schedule for Gugu Kali with ID: %', v_schedule_id;
  END IF;
END $$;
...
-- Step 4: Create bookings for bi-weekly Saturdays in Oct, Nov, Dec 2025
...
bi_weekly_dates AS (
  SELECT 
    (si.start_date + (generate_series(0, 70, 14) * interval '1 day'))::date as booking_date
  FROM schedule_info si
  WHERE si.start_date IS NOT NULL
)
...
SELECT 
  ...
  NOW() as created_at,
  NOW() as updated_at
FROM bi_weekly_dates bwd
CROSS JOIN schedule_info si
WHERE bwd.booking_date <= '2025-12-31' 
  AND bwd.booking_date >= '2025-10-01'
  AND EXTRACT(DOW FROM bwd.booking_date) = 6; -- Ensure it's a Saturday
...
-- Step 7: Show booking counts per month
SELECT 
  COALESCE(cl.name, 'Unassigned') as cleaner_name,
  CASE 
...
-- Step 8: Show all bookings by date
SELECT 
  b.booking_date,
