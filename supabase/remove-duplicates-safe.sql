-- ============================================
-- SAFE REMOVAL OF DUPLICATE RECURRING SCHEDULES
-- ============================================
-- Based on analysis:
-- - Lynne Thorpe: Keep schedule 672e21cf (Ethel Chizombe, 18 bookings, Oct-Dec)
--                  Remove schedule 6bc23b47 (Nyasha Mudani, same dates - duplicate)
--                  Remove schedule c2439a66 (Nyasha Mudani, Nov only - 8 bookings)
-- - All other duplicates: Keep oldest with most bookings, remove newer ones
-- ============================================
...
SELECT 
  'PREVIEW: Schedules to be removed' as action,
  str.schedule_id,
  c.first_name || ' ' || c.last_name as customer_name,
  rs.service_type || ', ' || rs.bedrooms || ' bed, ' || rs.bathrooms || ' bath' as service,
  COUNT(b.id) as bookings_to_transfer
...
LEFT JOIN bookings b ON b.recurring_schedule_id = str.schedule_id
GROUP BY str.schedule_id, c.first_name, c.last_name, rs.service_type, rs.bedrooms, rs.bathrooms
