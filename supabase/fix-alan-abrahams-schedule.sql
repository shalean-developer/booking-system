-- ============================================
-- FIX ALAN ABRAHAMS'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Every Thursday (weekly)
-- Cleaner: Mitchell Piyo
-- Total Cost: R388.00 (38800 cents)
-- Cleaner Earnings: R250 (25000 cents)
-- Start Date: October 2, 2025 (from image)
...
WHERE c.email = 'abrahamsalan@yahoo.com'
...
SET 
  frequency = 'weekly',
  day_of_week = 4, -- Thursday = 4
  preferred_time = '09:00:00',
  start_date = '2025-10-02',
  total_amount = 38800, -- R388.00 in cents
  cleaner_earnings = 25000, -- R250 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Mitchell Piyo' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'abrahamsalan@yahoo.com' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Mitchell Piyo' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      v_cleaner_id,
      true as is_active,
      '2025-10-02' as start_date,
      38800 as total_amount, -- R388.00 in cents
    ...
    RAISE NOTICE 'Created new schedule for Alan Abrahams with ID: %', v_schedule_id;
  END IF;
END $$;
...
-- Step 4: Create bookings for all Thursdays starting from Oct 2, 2025
WITH date_series AS (
  SELECT generate_series(
    '2025-10-02'::date, -- Start from Oct 2, 2025
    '2025-12-31'::date,
    '1 day'::interval
...
thursday_dates AS (
  SELECT booking_date
  FROM date_series
  WHERE EXTRACT(DOW FROM booking_date) = 4 -- Thursday = 4
),
alan_schedule AS (
  SELECT 
    rs.id as schedule_id,
...
SELECT 
  ...
  as_schedule.customer_id,
  as_schedule.customer_name,
  as_schedule.customer_email,
  as_schedule.customer_phone,
  as_schedule.service_type,
  td.booking_date,
  as_schedule.booking_time,
  as_schedule.address_line1,
  as_schedule.address_suburb,
  as_schedule.address_city,
  as_schedule.total_amount_cents,
  as_schedule.cleaner_earnings_cents,
  as_schedule.cleaner_id,
  as_schedule.schedule_id,
  'pending' as status,
  jsonb_build_object(
    'service_type', as_schedule.service_type,
    'bedrooms', as_schedule.bedrooms,
    'bathrooms', as_schedule.bathrooms,
    'extras', '[]'::jsonb,
    'extrasQuantities', '{}'::jsonb,
    'subtotal', ROUND(as_schedule.total_amount_cents / 100.0 - 50, 0),
    'serviceFee', 50,
    'frequencyDiscount', 0,
    'total', ROUND(as_schedule.total_amount_cents / 100.0, 2),
    'snapshot_date', NOW()::text,
    'manual_pricing', true
  ) as price_snapshot,
  NOW() as created_at,
  NOW() as updated_at
FROM thursday_dates td
CROSS JOIN alan_schedule as_schedule;
