-- ============================================
-- FIX MONGEZI'S RECURRING SCHEDULE
-- ============================================
-- Schedule: Every Monday, Wednesday, and Friday (custom-weekly)
-- Cleaner: Silibaziso Moyo
-- Total Cost: R500.00 (50000 cents)
-- Cleaner Earnings: R280 (28000 cents)
...
WHERE c.email = 'mongezib@arcfyre.com'
  AND rs.is_active = true
GROUP BY rs.id, c.first_name, c.last_name, cl.name, rs.service_type, rs.bedrooms, rs.bathrooms, rs.frequency, rs.preferred_time, rs.days_of_week, rs.start_date, rs.total_amount, rs.cleaner_earnings
...
SET 
  ...
  days_of_week = ARRAY[1, 3, 5], -- Monday = 1, Wednesday = 3, Friday = 5
  preferred_time = '07:00:00', -- From image: Custom Weekly at 07:00:00
  start_date = '2025-10-01',
  total_amount = 50000, -- R500.00 in cents
  cleaner_earnings = 28000, -- R280 in cents
  cleaner_id = (SELECT id FROM cleaners WHERE name = 'Silibaziso Moyo' LIMIT 1),
  updated_at = NOW()
WHERE id IN (
  SELECT rs.id
  FROM recurring_schedules rs
...
BEGIN
  -- Get customer ID
  SELECT id INTO v_customer_id FROM customers WHERE email = 'mongezib@arcfyre.com' LIMIT 1;
  
  -- Get cleaner ID
  SELECT id INTO v_cleaner_id FROM cleaners WHERE name = 'Silibaziso Moyo' LIMIT 1;
  ...
  IF v_existing_count = 0 AND v_customer_id IS NOT NULL AND v_cleaner_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      1 as bedrooms,
      1 as bathrooms,
      'custom-weekly' as frequency,
      ARRAY[1, 3, 5] as days_of_week, -- Monday, Wednesday, Friday
      '07:00:00' as preferred_time,
      v_cleaner_id,
      true as is_active,
      '2025-10-01' as start_date,
      50000 as total_amount, -- R500.00 in cents
      28000 as cleaner_earnings, -- R280 in cents
    ...
    RAISE NOTICE 'Created new schedule for Mongezi with ID: %', v_schedule_id;
  END IF;
END $$;

-- Step 3: Delete existing bookings for Oct, Nov, Dec 2025
...
-- Step 4: Create bookings for all Mondays, Wednesdays, and Fridays in Oct, Nov, Dec 2025
WITH date_series AS (
...
monday_wednesday_friday_dates AS (
  SELECT booking_date
  FROM date_series
  WHERE EXTRACT(DOW FROM booking_date) IN (1, 3, 5) -- Monday = 1, Wednesday = 3, Friday = 5
),
mongezi_schedule AS (
  SELECT 
    rs.id as schedule_id,
...
SELECT 
  ...
  ms.address_suburb,
  ms.address_city,
  ms.total_amount_cents,
  ms.cleaner_earnings_cents,
  ms.cleaner_id,
  ms.schedule_id,
  'pending' as status,
  jsonb_build_object(
    'service_type', ms.service_type,
    'bedrooms', ms.bedrooms,
    'bathrooms', ms.bathrooms,
    'extras', '[]'::jsonb,
    'extrasQuantities', '{}'::jsonb,
    'subtotal', ROUND(ms.total_amount_cents / 100.0 - 50, 0),
    'serviceFee', 50,
    'frequencyDiscount', 0,
    'total', ROUND(ms.total_amount_cents / 100.0, 2),
    'snapshot_date', NOW()::text,
    'manual_pricing', true
  ) as price_snapshot,
  NOW() as created_at,
  NOW() as updated_at
FROM monday_wednesday_friday_dates mwfd
CROSS JOIN mongezi_schedule ms;
