...
export function AdminQuotesView() {
  ...
  const columns: Column<Quote>[] = [
    ...
    },
    {
      id: 'service',
      header: 'Service',
      accessor: (row) => <span className="text-sm text-gray-900">{row.service_type}</span>,
    },
    {
      id: 'amount',
      header: 'Amount',
    ...
    },
    {
      id: 'date',
      header: 'Date',
      accessor: (row) => <span className="text-sm text-gray-600">{formatDate(row.created_at)}</span>,
    },
    {
      id: 'actions',
      header: 'Actions',
      accessor: (row) => (
        <div className="flex items-center gap-2">
          {row.status === 'pending' && (
            <>
              <Button variant="outline" size="sm" onClick={() => handleStatusUpdate(row.id, 'contacted')}>
                Mark Contacted
              </Button>
              <Button variant="default" size="sm" onClick={() => handleConvertToBooking(row.id)}>
