...
export function Stepper({ currentStep }: StepperProps) {
  ...
  const currentGroup = Math.ceil(currentStep / 2); // 1-2 = Group 1, 3-4 = Group 2, 5-6 = Group 3
  
  const getSubstep = (step: number) => {
    // Determine which substep within the group (1 or 2)
    return ((step - 1) % 2) + 1;
  };

  const getProgressPercentage = (step: number) => {
    // Convert 6-step system to 3-group progress (33%, 66%, 100%)
    if (step <= 2) return 33;
    if (step <= 4) return 66;
    return 100;
  };
