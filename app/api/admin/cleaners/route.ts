...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sort === 'completed_bookings') {
      let q = supabase.from('cleaners').select('*');
      if (active === 'true') {
        q = q.eq('is_active', true);
      } else if (active === 'false') {
        q = q.eq('is_active', false);
      }
      const { data: cleaners, error } = await q.limit(500);
      if (error) {
      ...
      const withStats = await attachCleanerStats(supabase, cleaners || []);
      withStats.sort((a: any, b: any) => (b.completed_bookings || 0) - (a.completed_bookings || 0));
      const total = withStats.length;
      const paged = withStats.slice(offset, offset + limit);
      return NextResponse.json({
        ok: true,
        cleaners: paged,
        total,
        ...
        sort: 'completed_bookings',
      });
    }

    let query = supabase.from('cleaners').select('*').order('created_at', { ascending: false });
    ...
    const cleanersWithStats = await attachCleanerStats(supabase, cleaners || []);
