...
export async function GET(req: Request) {
  ...
  try {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('relation') && error.message.includes('does not exist')) {
          ...
          diagnostics.errors.push(
            'Row Level Security (RLS) is blocking access. Check RLS policies for the services table.',
          );
        }
      }
      ...
      diagnostics.errors.push(`Database query failed: ${msg}`);
    }
    ...
    try {
      ...
      if (servicesError) {
        ...
        diagnostics.errors.push(
          'No active services found in the database. The services table is empty or has no active services.',
        );
      }
    } catch (servicesError: unknown) {
      const msg = servicesError instanceof Error ? servicesError.message : 'Unknown error';
      diagnostics.checks.servicesTable = {
        success: false,
        error: msg,
      };
      diagnostics.errors.push(`Services query failed: ${msg}`);
    }
    ...
    diagnostics.errors.push(`Diagnostic check failed: ${msg}`);
