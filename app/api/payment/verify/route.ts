...
import { handlePaystackVerifyPoll } from '@/lib/paystack-verify-poll';
...
export async function GET(req: Request) {
  try {
    ...
    if (isPoll) {
      return handlePaystackVerifyPoll(req);
    }
    ...
    if (isBookingPaidInDatabase(row)) {
      ...
      console.log('[api/payment/verify GET] booking already paid (DB)', row.id);
      return NextResponse.json({
        ok: true,
        success: true,
        duplicate: true,
    ...
    return NextResponse.json(
      {
        ...
        status: 'pending',
        message:
          'Payment is not confirmed in our records yet. If you completed checkout, wait a few seconds — confirmation is applied when Paystack notifies our server.',
      },
      { status: 200 },
    );
  } catch (e) {
